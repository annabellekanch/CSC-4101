...
class Environment(Node):
    ...
    def define(self, id, value):
        ...
        val = Environment.__find(id, self.__scope)
        if val is None:
            self.__scope = Cons(Cons(id, Cons(value, Nil())), self.__scope)
        else:
            val.setCar(value)
    ...
    def assign(self, id, value):
        ...
        if value is None and env is None:
            ...
            self.getErrorMessages().append(err)
            # System.out.println(errorMessages.size())
            # System.out.println("hello")
        elif val is None:
            self.assign(id, self.__env)
        else:
            val.setCar(Cons(id, value))
